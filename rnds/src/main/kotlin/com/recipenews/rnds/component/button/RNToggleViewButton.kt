...
/**
 * RecipeNews view toggle button with included trailing icon as well as compact and expanded
...
fun RNToggleViewButton(
    ...
    TextButton(
        ...
        contentPadding = RNToggleViewButtonDefaults.RNToggleViewButtonContentPadding,
    ) {
        RNToggleViewButtonContent(
            text = if (expanded) expandedText else compactText,
            trailingIcon = {
                Icon(
                    imageVector = if (expanded) RNIcons.ViewDay else RNIcons.ShortText,
...
/**
 * Internal RecipeNews view toggle button content layout for arranging the text label and
 * trailing icon.
 *
 ...
 */
@Composable
private fun RNToggleViewButtonContent(
...
@ThemePreviews
@Composable
fun RNToggleViewButtonPreviewExpanded() {
    RNTheme {
        Surface {
            RNToggleViewButton(
                expanded = true,
...
@Preview
@Composable
fun RNToggleViewButtonPreviewCompact() {
    RNTheme {
        Surface {
            RNToggleViewButton(
                expanded = false,
...
/**
 * RecipeNews view toggle default values.
 */
object RNToggleViewButtonDefaults {
    // TODO: File bug
    // Various default button padding values aren't exposed via ButtonDefaults
    val RNToggleViewButtonContentPadding =
        PaddingValues(
            start = 16.dp,
            top = 8.dp,
            end = 12.dp,
